...
function DepartmentsTab({ accessToken, router }: { accessToken: string | null; router: ReturnType<typeof useRouter> }) {
  ...
  async function handleToggleActive(dept: DeptItem, e: React.MouseEvent) {
    e.stopPropagation();
    if (!accessToken) return;
    try {
          ...
          {departments.map((dept) => (
            ...
            >
              <div className="flex items-start justify-between mb-3">
                ...
                >
                  <h3 className="text-base font-semibold text-slate-900 hover:text-blue-600 transition">{dept.name}</h3>
